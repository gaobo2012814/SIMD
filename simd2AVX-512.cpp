...
char** matrix_alloc(int row, int col, int align) {
    ...
    for (int i = 0; i < row; i++) {
        rows[i] = (char*)_aligned_malloc(col * sizeof(char), 64);
    }
    return rows;
}
...
vector<unsigned int > change(vector<vector<unsigned int>>temp, int maxreal)
{
    ...
    {
        for (int j = 0; j < maxreal; j += 64)
        {
            __m512 t1, t2, t3;
            t1 = _mm512_loadu_ps((const float*)(p + j));
            t2 = _mm512_loadu_ps((const float*)(ch[i] + j));
            t3 = _mm512_and_ps(t1, t2);
            _mm512_store_ps((float*)(p + j), t3);
        }
